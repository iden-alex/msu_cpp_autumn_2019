...
BigInt::BigInt(long long a):len(0), sign(a >= 0), ptr(nullptr) {
    if (!a) {
        len = 1;
        ptr = new char[1];
        ptr[0] = 0;
...
BigInt BigInt::Add_pos_arg(const BigInt &b) const {
    if (len >= b.len) {
        bool diff = false;
        for (size_t i = 0; i < len; ++i) {
...
BigInt BigInt::Add_neg_arg(const BigInt &b) const {
    size_t k = 0;
    bool diff = false;
    char buf[len + 1];
    for (size_t i = 0; i < len; ++i) {
...
bool BigInt::Is_mod_greater(const BigInt &b) const {
    if (len == b.len) {
